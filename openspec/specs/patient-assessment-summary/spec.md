...
The thank-you screen SHALL render a card titled **Avaliação preliminar** that displays the latest agent response directly, shows a loading indicator while the agent reply is pending, and surfaces an inline error message if the agent request fails. The card SHALL replace the legacy “Ver resultados” button and still include the “Adicionar informações” action that navigates to the demographic/report workflow.

#### Scenario: Agent result is available
- **WHEN** the thank-you screen loads and the agent response finishes successfully
- **THEN** the **Avaliação preliminar** card MUST display the agent text (e.g., a summary paragraph, insights, or flags)
- **AND** the “Adicionar informações” button MUST remain visible below the agent text

#### Scenario: Agent result is still running
- **WHEN** the thank-you screen loads before the agent response arrives
- **THEN** the **Avaliação preliminar** card MUST show a spinner and a brief “Processando análise” message
- **AND** it MUST prevent the agent text from being shown until the request completes

#### Scenario: Agent request fails
- **WHEN** the agent request returns an error or times out
