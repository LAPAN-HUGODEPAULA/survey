...
O repositório contém quatro subprojetos:
...
*   **`survey_api`**: Uma versão do backend para deploy no Firebase Functions.
...
*   **Dados Estáticos**:
    *   **Questionários**: `assets/surveys/*.json`. Usados pelo script de migração.
...
### Iniciar Backend (Deploy no Firebase)
'''bash
cd survey_api
